...
<div id="modal_add_identification" class="modal fade">
    <div class="modal-dialog modal-l">
        <div class="modal-content">
            <div class="modal-header  bg-light border-0">
                <h6 class="modal-title text-dark">Add New ID Type</h6>
                <button type="button" class="btn-close btn-danger" data-bs-dismiss="modal"></button>
            </div>
            <form class="form-horizontal" id="add_identification_form">
                @csrf
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-2">
                            <div class="col-md-12 mb-3">
                                <label class="form-label">Identification Name</label>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-secondary">Add ID Type</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    $("#add_identification_form").submit(function(e) {
        e.preventDefault();
        $.ajax({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            url: "{{route('identification.store')}}",
            type: "POST",
            data: $(this).serialize(),
            success: function(response) {
                ...
                if (response) {
                    ...
                    $("#add_identification_form")[0].reset();
                    $('#tbody').load(document.URL + ' #tbody tr');
                    $("#modal_add_identification").modal("hide");
                    new Noty({
