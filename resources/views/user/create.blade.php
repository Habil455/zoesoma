...
@section('content')
    <form id="add_user_form" action="{{route('user.store')}}" enctype="multipart/form-data" autocomplete="off" method="post" data-parsley-validate>
        @csrf
        ...
        <script>
            // Add user
            $("#add_user_for").submit(function(e) {
                e.preventDefault();
                $.ajax({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    url: "{{ route('user.store') }}", // Add the correct URL here
                    type: "POST",
                    data: $(this).serialize(),
            ...
            function loadPosition(value) {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        dropdown.append($("<option />").val('').text('Select Position'));
                        $.each(response, function(i, item) {
                            dropdown.append($("<option />").val(item.id).text(item.name));
                        });
                    }
